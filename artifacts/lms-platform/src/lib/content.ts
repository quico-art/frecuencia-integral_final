...
const DRAFT_KEY  = "lms_draft";
const PUB_KEY    = "lms_published";
...
export function getContentValue(key: string, fallback: string): string {
  const d = getDraft(); if (d[key] !== undefined) return d[key];
  const p = getPublished(); if (p[key] !== undefined) return p[key];
